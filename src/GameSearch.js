        ...
        return (
            <div className="mt-3">
                ...
                <table className="table table-striped">
                    ...
                    <TableBody handleLinkClick={props.handleLinkClick} games={props.games} sortBy={props.sortBy} stores={props.stores} deals={props.deals} priceRange={props.priceRange} storeFilter={props.storeFilter} onSale={props.onSale} />
                </table>
            </div>
        )
                ...
                for (let j = 0; j < props.stores.length; ++j) {
                    if (deal.gameInfo.storeID === props.stores[j].storeID) {
                        ...
                        if ((deal.gameInfo.salePrice > props.priceRange[0] && deal.gameInfo.salePrice < props.priceRange[1]) && (inFilter || props.storeFilter.length == 0) && (!props.onSale || deal.gameInfo.salePrice < deal.gameInfo.retailPrice))
                            return (<TableRow handleLinkClick={clickHandler} game={game} key={index} store={store} deal={deal} onSale={props.onSale} />)
                    }
                }
                break;
            }
...
const TableRow = (props) => {
    ...
    if (flag) {
        return (
            <tr>
                <td><Link to={`/game-detail/${g.gameID}`} onClick={()=>props.handleLinkClick(g.gameID)}><img src={g.thumb} alt={g.internalName} width={40} height={40} /></Link></td>
