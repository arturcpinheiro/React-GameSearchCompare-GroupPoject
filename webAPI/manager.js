...
module.exports = function() {
    ...
    return {
        ...
        gameGetById: function(gameId2){
            return new Promise((resolve, reject)=>{
                games.find({gameId: gameId2}, (err, result)=>{
                    console.log(gameId2)
                    if (err)
                        reject(err);
        ...
        gameUpdate: function(gameId, updatedGame){
            ...
            return new Promise((resolve, reject)=>{
                games.findOneAndUpdate(
                    ...
                    {  
                        $inc: {views: +1}
                        //views: updatedGame.views  //properties to be updated, add more properties later here
                    }, (err, result)=>{
                        console.log(result);
                        if (err)
                            reject(err);
        ...
        gameGetAll: function(){
            return new Promise((resolve, reject) =>{
                games.find({$orderby: { age : -1 }}, (error, results) => {
                    if (error)
                    reject(err);
